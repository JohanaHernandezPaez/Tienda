...
public class UsuariosDAO {
	...
	public UsuariosDTO consultar(UsuariosDTO cedula_usuario) {
		...
		try {
			...
			rs = ps.executeQuery();
			if (rs.next()) {
				usuariosdto = new UsuariosDTO(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getString(4),
						rs.getString(5));
			} else {
				return null;
			}
	...
	public int actualizar(UsuariosDTO usuario) {
		...
		try {
			ps = cnn.prepareStatement(
					"UPDATE usuarios SET nombre_usuario=?, email_usuario=?, password=?, usuario=? WHERE cedula_usuario=?");
